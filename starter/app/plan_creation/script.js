  ...
  searchForm.addEventListener("submit", (e) => {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        if (data.forecast) {
          display7DayForecast(data.forecast, startDate, endDate);
        }
      })
      .catch(error => {
...
function display7DayForecast(forecastData, startDate, endDate) {
  weatherContent = document.getElementById("weatherContent");
  if (!weatherContent) return;

  dailyForecasts = processForecastData(forecastData.list, startDate, endDate);
...
function processForecastData(forecastList, startDate, endDate) {
  ...
  dailyForecasts = Object.keys(dailyData)
    .map(dateKey => {
